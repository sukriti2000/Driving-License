const {getDriverInfoByUsername,saveDriver,ValidateDriver,getResultByUsername} = require('../../models/driver.model');

async function getGPage(req,res){
    try {
        const data = await getDriverInfoByUsername(req.cookies.username);
       
        res.render('G_page', { data }); // Pass the 'data' object as a property of an object
      } catch (err) {
        // Handle the error appropriately, e.g., send an error page
        res.status(500).send('Error retrieving data');
      }
    }
async function httpSaveDriverG(req,res){
...
async function httpGetResult(req,res){
  try {
    const response = await getResultByUsername(req.cookies.username,'G');
